...
try {
  $email = "mumbai_test_" + (Get-Random) + "@example.com"
  ...
  # 2. Call AI generate-itinerary for Mumbai with coordinates
  $generateBody = @{
  ...
  Write-Host "Generating Mumbai itinerary..."
  ...
  if ($allUnique) {
    Write-Host "SUCCESS: Mumbai days are unique! ✅"
  } else {
    Write-Host "FAILURE: Mumbai has duplicate days! ❌"
  }
