...
try {
  $email = "chicago_test_" + (Get-Random) + "@example.com"
  ...
  # 2. Call AI generate-itinerary for Chicago with coordinates
  $generateBody = @{
  ...
  Write-Host "Generating Chicago itinerary..."
  ...
  if ($allUnique) {
    Write-Host "SUCCESS: Chicago days are unique! ✅"
  } else {
    Write-Host "FAILURE: Chicago has duplicate days! ❌"
  }
